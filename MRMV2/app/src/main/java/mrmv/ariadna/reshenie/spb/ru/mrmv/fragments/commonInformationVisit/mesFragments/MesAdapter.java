 ...
 */
public class MesAdapter extends CursorAdapter {

    public MesAdapter(Context context, Cursor cursor) {
    ...
    public void bindView(View oCurrentView, Context context, Cursor cursor) {
        ...
        String sMesId, sCode, sText;


        MenuHolder menuHolder = (MenuHolder) oCurrentView.getTag();

        if (menuHolder != null) {
            sCode = cursor.getString(cursor.getColumnIndex(MesForDiagnoses.CODE));
            sText = cursor.getString(cursor.getColumnIndex(MesForDiagnoses.TEXT));
            sMesId = cursor.getString(cursor.getColumnIndex(MesForDiagnoses.MESID));

            menuHolder.tvCode.setText(sCode);
            menuHolder.tvText.setText(sText);
            menuHolder.sMesId = sMesId;
        }
