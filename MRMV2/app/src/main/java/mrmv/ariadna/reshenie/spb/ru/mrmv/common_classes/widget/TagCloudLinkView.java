package mrmv.ariadna.reshenie.spb.ru.mrmv.common_classes.widget;
...
public class TagCloudLinkView extends RelativeLayout {
    ...
    private int iSelectedTagIndex;
    private HashMap<Integer, View> hmAllView;
    ...
    public void add(Tag tag) {
        ...
        iSelectedTagIndex = mTags.size() - 1;
    }
    ...
    public void drawTags() {
        ...
        for (Tag item : mTags) {
            ...
            hmAllView.put(position, tagLayout);

            if(iSelectedTagIndex == listIndex){
                tagLayout.setBackground(getResources().getDrawable(R.drawable.shape_selected));
            }else{
                tagLayout.setBackground(getResources().getDrawable(R.drawable.shape));
            }
...
//            tagLayout.setBackgroundColor(mTagLayoutColor);
            ...
            tagView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if( mSelectListener != null) {
                        ...
                        if(iSelectedTagIndex == position){
                            return;
                        }

                        iSelectedTagIndex = position;

                        if(hmAllView != null){
                            for(Integer iPositionView : hmAllView.keySet()){

                                if(iPositionView == position){
                                    hmAllView.get(position).setBackground(getResources().getDrawable(R.drawable.shape_selected));

                                }else{
                                    hmAllView.get(position).setBackground(getResources().getDrawable(R.drawable.shape));
                                }
                            }
                        }
    ...
    private void dialogDeleteTag(final Tag tag, final int position){
        AlertDialog.Builder alerDialog = new AlertDialog.Builder(getContext());
        alerDialog.setTitle("Подтверждение удаления");  // заголовок
        alerDialog.setMessage("Вы действительно хотите удалить запись?"); // сообщение

        alerDialog.setPositiveButton("Удалить", new DialogInterface.OnClickListener() {
    ...
    public void setiSelectedTagIndex (int iIndex){
        iSelectedTagIndex = iIndex;
    }
